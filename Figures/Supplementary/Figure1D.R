run_gtex_brain_corr <- function(
  ...
  all_degs_rds_path = NULL,         # optional (for DE overlay); if NULL, DE overlay is skipped
  ...
  # ---- Parameters ----
  cpm_thresh = 1,
  expr_prop  = 0.10,  # fraction of samples with CPM>=cpm_thresh to keep a gene
  fdr_alpha  = 0.05,
  min_common_subjects = 3,   # guard: skip pairs with too few overlapping subjects
  cores = 40,
  seed  = 1,
  force_recompute = FALSE     # ignore cache and recompute correlations
) {
...
res <- run_gtex_brain_corr(
  ...
  all_degs_rds_path          = "all_degs_test.RDS",  # keep if you want DE overlay; set NULL to skip
  
  # outputs
  output_dir = "/tmp",                         # where PDFs/TSVs go
  cache_dir  = "/tmp",                    # cache heavy results
  heatmap_pdf = "gtex_sig_pos_degs.pdf", # same filename you used

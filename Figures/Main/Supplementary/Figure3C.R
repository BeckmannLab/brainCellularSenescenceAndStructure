...
resFinal2 <- resFinal[!resFinal$feature %in% rows_to_remove, ]

resFinal2 = resFinal2[which(resFinal2$adj.P.Val <= 0.05),]

bulk = resFinal
bulk2<- bulk[order(bulk$adj.P.Val, decreasing = FALSE), ]

bulk2 = bulk2[which(bulk2$adj.P.Val <= 0.05),]

subsetdf = as.data.frame(table(bulk2$feature))
...
###prot
resFinal = fread("allSTRDEGresults_01.18.24.txt")
prot = resFinal
prot2<- prot[order(prot$adj.P.Val, decreasing = FALSE), ]

prot2 = prot2[which(prot2$adj.P.Val <= 0.05),]

subsetdf = as.data.frame(table(prot2$feature))
...
subsetdf3$new_var= subsetdf3$Var1
subsetdf3$new_var = gsub("_", " ", subsetdf3$new_var)
subsetdf3$new_var = gsub("rh", "RH", subsetdf3$new_var)
subsetdf3$new_var = gsub("lh", "LH", subsetdf3$new_var)
...
all_ds =rbind(bulk, prot)

all_ds$ds = gsub("bulk", "Bulk RNAseq", all_ds$ds)
all_ds$ds = gsub("prot", "MS",all_ds$ds)

# Create the plot
color_blind_palette <- c("#332288", "#EE7733")

g = ggplot(all_ds, aes(x = Freq, y = reorder(new_var, Freq, sum), color = ds)) +
...
ggsave("degs_prot_bulk_smri.pdf", g, width = 12, height = 8)

...
V1 = c("lbp_06.21.24_pseudobulk_10percent.RDS", "lbp_06.21.24_pseudobulk_10percent.RDS", "lbp_06.21.24_pseudobulk_10percent.RDS", "lbp_06.21.24_pseudobulk_10percent.RDS", "lbp_06.21.24_pseudobulk_10percent.RDS", "lbp_06.21.24_pseudobulk_10percent.RDS", "lbp_06.21.24_pseudobulk_10percent.RDS")

V2 = c("10percent", "10percent", "10percent", "10percent", "10percent", "10percent", "10percent")

V3 = c("pb_10percent", "pb_10percent", "pb_10percent", "pb_10percent", "pb_10percent", "pb_10percent", "pb_10percent")
...
V7 = c("0.4", "0.4", "0.4", "0.4", "0.4", "0.4", "0.4")
...
final_df = readRDS("clean_across_percent_fishers_results.RDS")
...
max_non_inf <- max(all_percentages_celltype$log_oddsratio[is.finite(all_percentages_celltype$log_oddsratio)], na.rm = TRUE)
all_percentages_celltype$log_oddsratio[is.infinite(all_percentages_celltype$log_oddsratio)] <- max_non_inf + 1

all_percentages_celltype = as.data.table(all_percentages_celltype)


seriation_result <- seriate(dist(all_percentages_celltype %>% select(celltype, test, log_oddsratio)), method = "OLO")
...
all_ordered2 = all_ordered[which(all_ordered$celltype!= "noneu"),]


all_ordered2 = all_ordered[which(all_ordered$celltype!= "noneu"),]
all_ordered2$celltype = gsub("mg", "MG", all_ordered2$celltype)
all_ordered2$celltype = gsub("opc", "OPC", all_ordered2$celltype)
all_ordered2$celltype = gsub("exc", "Exc", all_ordered2$celltype)
all_ordered2$celltype = gsub("ast", "Ast", all_ordered2$celltype)
all_ordered2$celltype = gsub("int", "Int", all_ordered2$celltype)
all_ordered2$celltype = gsub("oli", "Oli", all_ordered2$celltype)
...
d = ggplot(all_ordered2, aes(x = celltype, y = test, fill = log_oddsratio)) +
  ...
  labs(fill = "Log2(Odds Ratio)", x = "Cell Type", y = "Senescence cell proportion threshold") +
  theme_bw() +  
  theme(
  ...
  coord_fixed() +   guides(fill = guide_colorbar(frame.colour = "black", frame.linewidth = 0.2)) 


ggsave("PM_DE_oddsratio_heatmap_sen.pdf", d, width = 10, height = 10)

...
sample_name = "GO_prot_results"
plot_directory = "path"
map <- ##map between prot id and gene id
prot_results= #see supplementary table 2 MS sMRI DE

##format
prot_results=as.data.table(prot_results)
prot_results[,V1:=tstrsplit(V1, split=".", fixed=T, keep=1L)]
prot_results=as.data.frame(prot_results)
prot_with_map=merge(prot_results, map, by.x = "V1", by.y = "refseq")
prot_with_map <- na.omit(prot_with_map)

prot_with_map$DE = ifelse(prot_with_map$adj.P.Val <= 0.05, "Signif", "notSignif")

features = unique(prot_with_map$feature)

foreach (x = features) %do% {
	df = prot_with_map[which(prot_with_map$feature == x),]
	...
	print(x)
	gene.map = getgo(for_gsea_df$gene_ID,'hg19','ensGene')
	if (any(for_gsea_df$DE_new == 1)) {
	try(annotate_GOterms_DE(mel=for_gsea_df,sample_name=sample_name,gene.map=gene.map,rrvgo=TRUE,useFDR=TRUE,ensGene==TRUE))
	}
	else {
		print("no sig")
	}	
...
saveRDS(myres,"prot_sig_GO_all_features.RDS" )

...
# pb=pb; assays=ct.pairs; method = "random"; formula = ~0; collapse = TRUE; min.cells = 10; min.count = 10; min.prop=0.75; min.samples = 4; isCounts = TRUE; normalize.method = "TMM"; robust = FALSE; quiet = FALSE; contrasts = c(compare = paste("cellClustertest - cellClusterbaseline")); BPPARAM = SerialParam(); errorsAsWarnings = FALSE; useProcessOneAssay_edgeR 



dreamletCompareClusters_edgeR <- function(pb, assays, method = c("fixed", "random", "none"), formula = ~0, collapse = TRUE, min.cells = 10, min.prop=0.4,  min.count = 10, min.samples = 4, isCounts = TRUE, normalize.method = "TMM", robust = FALSE, quiet = FALSE, contrasts = c(compare = paste("cellClustertest - cellClusterbaseline")), BPPARAM = SerialParam(), errorsAsWarnings = FALSE, useProcessOneAssay_edgeR = FALSE, ...) {
  ...
  if (!all(all.vars(formula) %in% colnames(dreamlet:::get_metadata_aggr_means(pb)))) {
    i <- which(!all.vars(formula) %in% colnames(dreamlet:::get_metadata_aggr_means(pb)))
  ...
  if (collapse) {
    ...
    res <- lapply(names(assay.lst), function(clstrSet) {
      ...
      aggr_means <- as.data.frame(dreamlet:::get_metadata_aggr_means(pb))
      ...
      df <- dreamlet:::merge_metadata(data_constant[ids, , drop = FALSE], dftmp, clstrSet, metadata(pb)$agg_pars$by)
  ...
  if(useProcessOneAssay_edgeR==FALSE){
    form <- switch(method,
      "random" = {
      ...
      form = update.formula(formula, ~ 0 + cellCluster + Sample + .)
  }
  ...
  if(useProcessOneAssay_edgeR==FALSE){
    ...
    vobj <- tryCatch(
      {
        # processing counts with voom or log2 CPM
        processOneAssay_edgeR(countsMatrix, form, data, n.cells,
          min.cells = min.cells,
          min.count = min.count,
          min.prop=min.prop,
          min.samples = min.samples,
          ...
          normalize.method = normalize.method,
          quiet = quiet,
          BPPARAM = BPPARAM, useProcessOneAssay_edgeR = TRUE, ...
        )
      },
      error = function(e) {
        if (errorsAsWarnings) {
  ...
  if(useProcessOneAssay_edgeR==FALSE){
    ...
    returnFit=fit
      ...
      colnames(design)[1:2]=c("baseline","test")
      colnames(design)=make.names(colnames(design))

      fit = glmQLFit(vobj,design)
      contrast=makeContrasts(test - baseline, levels=design)
      qlfFit=glmQLFTest(fit,contrast=contrast)
      returnFit=qlfFit
    }
  returnFit
}

environment(dreamletCompareClusters_edgeR) <- rlang::new_environment(parent = environment(dreamlet::dreamletCompareClusters))



# res = as.data.frame(topTags(qlfFit, n = Inf))

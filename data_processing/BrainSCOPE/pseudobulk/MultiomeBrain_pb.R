...
all_sene = qread("all_Aucell_results_with_cutoffs_MultiomeBrain.qs") #see aucell
sces3 =readRDS("sce_all_MultiomeBrain.RDS") #see setup
...
resfinal=foreach(i = percentages) %do% {
    ...
    print(i)
    file = paste0(folder, paste0("MultiomeBrain_pseudobulk_", i, ".qs"))
    if (!file.exists(file)){

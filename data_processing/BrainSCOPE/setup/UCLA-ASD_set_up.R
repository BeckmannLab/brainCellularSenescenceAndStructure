...
for (expFile in expFiles) {
  ...
  if (!sample %in% duplicates) {
   ...
   if (sample == "5297"){
    meta_sample = PEC2_sample[PEC2_sample$Individual_ID == "HSB378_DFC", ]
    meta_sample$Cohort = "UCLA-ASD"
    meta_sample$Individual_ID = "5297"
    }else{
...
saveRDS(sces3,"sce_all_UCLA-ASD.RDS")
